...
library(roxygen2)
library(dplyr)
load_pkgload("/media/emunte/Nuevo/Variant_class/vaRHC") 
```
## Contents
...
## 2. Installation {#install}
vaRHC package can be installed via CRAN:
```{r, eval=FALSE}
install.packages("vaRHC")
```
The package can bee also be downloaded from Github:
```{r, eval=FALSE}
...
```{r echo=FALSE}
all.transcript <- connectionDB("SELECT ensembltranscriptID, namegene, NM, NC, CCDS from transcript WHERE maintranscript = 'T';") %>% as.data.frame() %>% dplyr::arrange(namegene)
...
```{r, echo=FALSE}
 gene.specific.df <- connectionDB("SELECT * from  gene_specific;") %>% as.data.frame()
...
##### 3.1.1.2.3 browser
Which browser to start Rselenium server. By default is "firefox" (the recommended). If you do not have firefox installed try either "chrome" or "phantomjs" (but they have not been tested for this package).
...
##### 3.1.1.2.5 Report related parameters
...
var.information <- vaR(assembly = eg.assembly, gene = eg.gene, variant = eg.variant, gene.specific.df = NULL)
...
```{r eval=FALSE}
eg.variants <- data.frame(gene=c("ATM", "MSH6", "BRCA2"), variants = c("c.8420A>T", "c.1559G>A", "c.6024dup"))
batch.results <- vaRbatch(assembly = "hg19", all.variants = eg.variants, spliceai.program = TRUE, spliceai.reference= "../docs/hg19.fa", excel.results = TRUE, path.original.file="../docs/template.xlsx")
```

#### 3.1.2 Output
It returns a list containing vaR() info for all variants and it also returns a log file, with the detailed time execution and any error that may occur explained. The log file is stored in package log folder.
