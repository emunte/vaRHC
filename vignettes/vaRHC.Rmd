...
```
If you use this option an extra step must be done: 
https://github.com/emunte/vaRHC/blob/main/extdata/hg38ToHg19.over.chain document must be downloaded and stored in extdata folder. 
To see the path of the folder paste in R the following command:
```{r, eval=FALSE}
path = system.file(package="vaRHC", "extdata")
```
...
- ***NM***: Accession number of the transcrit and mRNA from RefSeq. By default is NULL and vaRHC will consider the ones detailed above. Be careful if you use a different NM because the program has not been validated for it. If you provide a different NM, CCDS must also be provided.The transcript accession number is retrieved from RefSeq. By default, this function parameter is set to NULL. vaRHC will consider the NMs listed in the table above. If a different NM is used than the ones described in the table, it must be noted that the program has not been validated yet for it. In this case, a CCDS ID is also required.



- ***CCDS***: Consensus CDS ID retrieved from NCBI CCDS database (which can be found in the following link https://www.ncbi.nlm.nih.gov/projects/CCDS/CcdsBrowse.cgi). By default is set to NULL. vaRHC will consider the IDs detailed in the table above. Again, it must be noted that if a different CCDS ID is provided, the program has not been validated yet for it. In this case, NM accession number is required. 
...
genome.reference <- "path_to_hg19.fa"
all <- vaRbatch( all.variants = ex_vaRbatch, spliceai.program = TRUE, splieai.reference= genome.reference, excel.results = TRUE)
